...
impl Ebuild {
    ...
    pub async fn parse(path: PathBuf) -> Result<Self, String> {
        ...
        tokio::spawn(async move {
            stdin.write_all(SRC_URI_HELPER_PY.as_bytes()).await.expect("failed to write stdin");
        });

        let stdout = preprocessor.stdout.take().expect("failed to open stdout");
        let mut src_uri_json = String::new();
        io::BufReader::new(stdout).read_to_string(&mut src_uri_json).await.map_err(|e| e.to_string())?;
